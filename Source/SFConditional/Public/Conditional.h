...
namespace SF
{
	...
	class SFCONDITIONAL_API UConditional : public UObject
	{
		GENERATED_BODY()
	...
	public:
		...
		FConditionalAnswer EvaluateObject(UObject* TestObject);
		 ...
		 */
		UFUNCTION(BlueprintCallable, Category="Conditional")
		FConditionalAnswer Evaluate(const FConditionalEvaluationContext& EvaluationContext);
		...
		void ForThisAndEachChildDo(const TFunction<void(UConditional*)>& Work);

		/** Returns any immediate conditional children. */
		UFUNCTION(BlueprintNativeEvent, Category="Conditional")
		TArray<UConditional*> GetImmediateChildren() const;
	...
	protected:
		 ...
		 * Should never be called by any other method than UConditional::Evaluate(..).
		 */
		UFUNCTION(BlueprintNativeEvent, Category="Conditional")
		FConditionalAnswer EvaluateInternal(UPARAM(ref) const FConditionalEvaluationContext& EvaluationContext);
