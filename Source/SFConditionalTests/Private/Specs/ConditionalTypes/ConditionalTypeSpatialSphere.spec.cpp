...
void FConditionalTypeSpatialSphereSpec::Define()
{
	BeforeEach([this]
	{
		...
		TestObjectObject = InstigatorObject = NewObject<UConditional_MockObject>();
		
		TestObjectActor = TestWorld->AsRef().SpawnActor<AConditional_MockActor>();
		InstigatorActor = TestWorld->AsRef().SpawnActor<AConditional_MockActor>();
	});
	AfterEach([this]
	{
		TestWorld.Reset();
